...
class AsonConfig:
    ...
    # --- IP Allowlisting ---
    IP_ALLOWLIST: str = os.getenv("ASON_IP_ALLOWLIST", "")  # Comma-separated CIDRs, empty = allow all
    ...
    AUDIT_RETENTION_HOURS: int = int(os.getenv("AUDIT_RETENTION_HOURS", "2160"))  # 90 days

    # --- SLA Targets ---
    SLA_TARGET_P99_MS: float = float(os.getenv("SLA_TARGET_P99_MS", "30000"))
    SLA_TARGET_SUCCESS_RATE: float = float(os.getenv("SLA_TARGET_SUCCESS_RATE", "0.995"))
    ...
    def validate(self) -> List[str]:
        ...
        if self.ENVIRONMENT == "production" and not self.IP_ALLOWLIST:
            warnings.append("IP_ALLOWLIST empty in production — all IPs allowed")
        if self.JOB_RETENTION_HOURS < 24:
            warnings.append(f"JOB_RETENTION_HOURS={self.JOB_RETENTION_HOURS} — less than 24h may lose active jobs")
        return warnings
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "ip_allowlist_enabled": bool(self.IP_ALLOWLIST),
            "max_concurrent_jobs": self.MAX_CONCURRENT_JOBS,
            "job_retention_hours": self.JOB_RETENTION_HOURS,
            "audit_retention_hours": self.AUDIT_RETENTION_HOURS,
            "sla_p99_ms": self.SLA_TARGET_P99_MS,
            "sla_success_rate": self.SLA_TARGET_SUCCESS_RATE,
        }
